module.exports = (function() {


  //var zlib = require( 'zlib' );
  var mime = require( 'mime' );
  var fs = require( 'fs-extra' );
  var Promise = require( 'es6-promise' ).Promise;
  var extend = require( 'extend' );
  var E$ = require( 'emoney' );
  var Codes = require( './codes' );
  var gzip = require( './gzip' );
  //var ResponseThread = require( './response-thread' );


  var WARN = 'warn';
  var ERROR = 'error';
  var RESOLVED = 'resolved';


  function $Response( $req , res , options ) {
    
    var that = this;

    E$.construct( that );

    Object.defineProperties( that , {
      $original: {
        value: res
      },
      /*$req: {
        value: $req
      },*/
      $gzip: {
        value: (function() {
          var accept = $req.$original.headers[ 'Accept-Encoding' ] || '';
          return accept.indexOf( 'gzip' ) && options.gzip;
        }())
      }
    });

    that.$code = $req.$code;
    that.$headers = {};
    that.$target = $req.absolute().resPath;
    that.$stat = $req.$stat;
    that.$body = Codes.body( $req.$code );

    that.setHeaders(
      Codes.headers( $req.$code )
    );
  }


  $Response.prototype = E$.create({

    setHeaders: function( headers ) {
      var that = this;
      extend( that.$headers , headers );
    },

    digest: function() {
      var that = this;
      return new Promise(function( resolve ) {
        resolve( that._digest() );
      })
      .then(function( digestPromise ) {
        return digestPromise;
      })
      .then(function() {
        return that.$emit( RESOLVED , { code: that.$code });
      });
    },

    _stream: function() {

      var that = this;
      var stream = fs.createReadStream( that.$target );

      if (that.$gzip) {
        //stream = stream.pipe( zlib.createGzip() );
        //that.setHeaders({ 'Content-Length': gzipSize });
        that.setHeaders({ 'Content-Encoding': 'gzip' });
        /*var e$promise = gzip( that.$target , that.$stat );
        e$promise.$when( 'readable' , function( e , data ) {
          console.log(data);
        });
        return e$promise;*/
        return new gzip( that.$target , that.$stat );
      }

      return new Promise(function( resolve ) {
        resolve( stream );
      });
    },

    _digest: function() {

      var that = this;
      var original = that.$original;
      var code = that.$code;
      var headers = that.$headers;

      //that.setHeaders({ 'Content-Length': that.$stat.size });

      return new Promise(function( resolve , reject ) {

        if (that.$body) {
          original.writeHead( code , headers );
          original.end( that.$body );
          resolve();
        }
        else {
          /*that._stream().then(function( stream ) {
            stream.on( ERROR , reject );
            original.on( ERROR , reject );
            original.on( 'finish' , resolve );
            original.writeHead( code , headers );
            stream.pipe( original );
          });*/
          resolve( that._stream() );
        }
      })
      .then(function( freader ) {

        return new Promise(function( resolve , reject ) {
          
          original.on( ERROR , reject );
          //original.on( 'finish' , resolve );
          original.writeHead( code , headers );

          freader.stream.on( ERROR , reject );

          freader.$when( 'readable' , function( e , data ) {
            that.setHeaders({ 'Content-Length': data.size });
            resolve( freader );
          });
        });
      })
      .then(function( freader ) {
        freader.stream.pipe( original );
      })
      .catch(function( err ) {
        that.$emit( ERROR , err );
      });
    }

  });


  return $Response;


}());



















